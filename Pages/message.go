...
func MessagePage(window fyne.Window) fyne.CanvasObject { // 使用控件类型控制控件的位置和逻辑
	...
	var welcomeColor, noticeColor uint32
	if isDark {
		welcomeColor = 0x4A6D3D // 深色模式下较深的绿色
		noticeColor = 0x2F6F70  // 深色模式下较深的青色
	} else {
		welcomeColor = 0xBFEF93 // 浅色模式原色
		noticeColor = 0xA9EFEE  // 浅色模式原色
	}

	carouselItems := []*tools.CarouselItem{
		tools.NewCarouselItem("🚀 "+tools.GetLocalized("message_carousel_welcome_title"), tools.GetLocalized("message_carousel_welcome_subtitle"), welcomeColor, ""),
		tools.NewCarouselItem("🔔 "+tools.GetLocalized("message_carousel_notice_title"), fmt.Sprintf(tools.GetLocalized("message_carousel_notice_subtitle"), "2025-11-23-1"), noticeColor, ""),
