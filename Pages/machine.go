...
func MachinePage(window fyne.Window, contentStack *fyne.Container, mainContent fyne.CanvasObject) fyne.CanvasObject {
	...
	progressBar := widget.NewProgressBarWithData(progressBarBinding)
	progressBar.Hide()
	...
	startBtn := widget.NewButton(tools.GetLocalized("machine_start_exam"), func() {
		...
		go func() {
			...
			fyne.DoAndWait(func() {
				...
				// 进入考试界面
				window.SetContent(NewExamMainPage(window, zipSelect.Selected, contentStack, mainContent))
			})
		}()
...
func NewExamMainPage(window fyne.Window, examName string, contentStack *fyne.Container, mainContent fyne.CanvasObject) fyne.CanvasObject {
	// 设置自定义主题
	...
	// 主容器 - 添加自定义背景色
	background := canvas.NewRectangle(color.NRGBA{R: 0xF5, G: 0xF5, B: 0xDC, A: 0xFF}) // 米色背景
	...
	submitBtn := createStyledButton(tools.GetLocalized("machine_submit"), func() {
		dialog.ShowConfirm(tools.GetLocalized("machine_submit_confirm_title"), tools.GetLocalized("machine_submit_confirm_content"), func(confirmed bool) {
			if confirmed {
				submitExam(window, examState, contentStack, mainContent, examData, examTempPath)
			}
		}, window)
	})
	...
	giveUpBtn := createStyledButton(tools.GetLocalized("machine_giveup"), func() {
		dialog.ShowConfirm(tools.GetLocalized("machine_giveup_confirm_title"), tools.GetLocalized("machine_giveup_confirm_content"), func(confirmed bool) {
			if confirmed {
				...
				contentStack.Objects = []fyne.CanvasObject{MachinePage(window, contentStack, mainContent)}
			}
		}, window)
	})
	...
	for idx, drGroup := range drGroups {
		...
		}, window, tools.GetLocalized)

		// 手动设置卡片大小 - 使用配置值
		card.Resize(fyne.NewSize(resConfig.CardWidth, resConfig.CardHeight))

		// 创建带背景的卡片容器
		cardBackground := canvas.NewRectangle(color.NRGBA{R: 0xF5, G: 0xF5, B: 0xDC, A: 0xFF}) // 米色
	...
	cardContainer.Resize(fyne.NewSize(resConfig.ScrollWidth, float32(totalCards)*resConfig.CardSpacing))
...
// submitExam 交卷处理
func submitExam(window fyne.Window, examState *tools.ExamState, contentStack *fyne.Container, mainContent fyne.CanvasObject, originalExamData *tools.ExamData, examTempPath string) {
	finalScore, totalScore, correctCount, wrongCount := examState.CalculateScore()

	// 显示等待对话框
	waitDialog := dialog.NewCustom(tools.GetLocalized("machine_wait_wrong_set"), tools.GetLocalized("machine_wait_wrong_set_cancel"), widget.NewLabel(tools.GetLocalized("machine_wait_wrong_set_label")), window)
	waitDialog.Show()
	...
	go func() {
		...
		fyne.DoAndWait(func() {
			...
			if err != nil {
				dialog.ShowError(errors.New(tools.GetLocalized("machine_save_result_failed")+err.Error()), window)
				return
			}
			...
			dialog.ShowInformation(tools.GetLocalized("machine_exam_result_title"),
				fmt.Sprintf(tools.GetLocalized("machine_exam_result_content"),
					finalScore, totalScore, correctCount, wrongCount), window)
			window.SetContent(mainContent)
			contentStack.Objects = []fyne.CanvasObject{MachinePage(window, contentStack, mainContent)}
		})
	}()
